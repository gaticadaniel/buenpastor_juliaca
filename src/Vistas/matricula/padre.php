<div name="menu">
	<ul class="nav nav-tabs nav-justified">
		...
		<li class='active'><a href='#' onclick='javascript:document.form_padre.submit()'>Padre <?php if(isset($_SESSION['padreMatri'])) echo "<span class='glyphicon glyphicon-ok'></span>"?></a></li>
...
<div class="container">
	<form class="form-inline" action="../../src/Controladores/MatriculaControl.php" method="post" onsubmit="return validacion()" name="buscar">
		<div class="form-group">
			<h3>Buscar Padre de Familia <small> Complete el formulario:
			<input type="text" class="form-control" name="dniPadre" id="dniPadre" placeholder="DNI" required/>	
			<input type="hidden" name="control" id="control" value="BuscarPadre" >
...
if( isset($_SESSION['padreMatricula']) or isset($_SESSION['dniPadre']) or isset($_SESSION['padreMatri']) ){
	if ( isset($_SESSION['padreMatricula']) ){
		$padreMatricula = $_SESSION['padreMatricula'];
			
		$id=$padreMatricula['0']['id'];
		$dni=$padreMatricula['0']['dni'];
		$nombres=$padreMatricula['0']['nombres'];
		$apellido_paterno=$padreMatricula['0']['apellido_paterno'];
		$apellido_materno=$padreMatricula['0']['apellido_materno'];
		$sexo=$padreMatricula['0']['sexo'];
		$role=$padreMatricula['0']['role'];
		$padre=$padreMatricula['0']['padre'];
		$madre=$padreMatricula['0']['madre'];
		$apoderado=$padreMatricula['0']['apoderado'];
		$estado=$padreMatricula['0']['estado'];
		$direccion=$padreMatricula['0']['direccion'];
		$movistar=$padreMatricula['0']['movistar'];
		$rpm=$padreMatricula['0']['rpm'];
		$claro=$padreMatricula['0']['claro'];
		$otro=$padreMatricula['0']['otro'];
		$fijo=$padreMatricula['0']['fijo'];
		$fecha_nacimiento=$padreMatricula['0']['fecha_nacimiento'];
		$comentario=$padreMatricula['0']['comentario'];
		$created=$padreMatricula['0']['created'];
		$modified=$padreMatricula['0']['modified'];
		$username=$padreMatricula['0']['username'];
		$password=$padreMatricula['0']['password'];
		$accion="MODIFICAR";
//		$_SESSION['padreMatricula']=null;
	} 
	if ( isset($_SESSION['padreMatri']) ){
		$alumnoMatricula = $_SESSION['padreMatri'];
...
?>
			<form class='form-horizontal' method="POST"  name="form_padre" id="form_padre" action="../Controladores/MatriculaControl.php" onSubmit="return validando()">
							...
							<div class="form-group">
								...
								<div class="col-lg-10">
									<div class="radio">
									  <label>
									    <input type="radio" name="sexo" id="sexo" value="Masculino" checked="true" >Masculino
									  </label>
									  <label>
									    <input type="radio" name="sexo" id="sexo" value="Femenino" disabled="true" >Femenino
									  </label>
									</div>
								</div>
							</div>
				...
				<div class="form-group">
					<div class="col-lg-offset-2 col-lg-10">
						...
						<input type="hidden" name="control" id="control" value="ConfirmarPadre" >
						<button type="submit" class="btn btn-default" <?php if(!isset($_SESSION['alumnoMatri'])) echo "disabled='true'"?> >Confirmar</button>
					</div>
				</div>
...
<script language="javascript" type="text/javascript"> 
	function validacion() { 
		var dni=document.getElementById("dniPadre").value;
			if (!(/^\d{8}$/.test(dni))) { 
				alert('DNI no válido');
