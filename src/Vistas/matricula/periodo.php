<div name="menu">
	<ul class="nav nav-tabs nav-justified">
		...
		<li class='active'><a href='#' onclick='javascript:document.form_periodo.submit()'>Periodo <?php if(isset($_SESSION['periodoMatri'])) echo "<span class='glyphicon glyphicon-ok'></span>"?></a></li>
...
<?php
	if(isset($_SESSION['periodosMatricula'])){
		$array = $_SESSION['periodosMatricula'];
		
		echo "<h3>Periodos activos: <small>".count($array)." encontrado(s)</small></h3>";
		if (count($array) > 0) {
			$COLUMNAS = 5;
			$i=0;
			...
			foreach ($array as $key => $value) {
				...
				echo "<tr>";
				foreach ($value as $titulo => $contenido) {
					if ($titulo=='id') $periodo_id = $contenido;
					if ($i < $COLUMNAS) echo "<td>".$contenido."</td>";
					else break;
					$i++;
				}
				if( isset($_SESSION['periodoMatri']) ){
					if($_SESSION['periodoMatri']['id']==$periodo_id){
						echo "<td><label><input type='radio' name='periodo' value='".$periodo_id."' checked></label></td>";
					} else {
						echo "<td><label><input type='radio' name='periodo' value='".$periodo_id."'></label></td>";
					}
				} else {
					echo "<td><label><input type='radio' name='periodo' value='".$periodo_id."'></label></td>";
				}
...
?>
	<input type='hidden' name='control' id='control' value='ConfirmarPeriodo'>
	<button type="submit" class="btn btn-default" <?php if(!isset($_SESSION['apoderadoMatri'])) echo "disabled='true'"?> >Confirmar</button>
</form>
...
function validate() {
	var a = 0, rdbtn=document.getElementsByName("periodo")
